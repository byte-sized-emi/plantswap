...
impl<P: PlantRecogniser> Backend<P> {
    pub async fn new(config: &AppConfig) -> Self {
        ...
        let head_bucket_response = s3_client.head_bucket().bucket(&images_bucket).send().await;
    ...
    pub async fn delete_all(&self) -> BackendResult<()> {
        ...
        diesel::delete(listings::table).execute(&mut *con)?;
        ...
        self.s3_client
            ...
            .send()
            .await
            .map_err(Into::<aws_sdk_s3::Error>::into)?;
    ...
    pub async fn upload_image(&self, user: Uuid, image: Bytes) -> BackendResult<Uuid> {
        ...
        new_image
            .insert_into(crate::schema::images::table)
            .execute(&mut *con)?;
    ...
    pub async fn get_image(&self, image: Uuid) -> BackendResult<Option<(String, Bytes)>> {
        ...
        match result {
            ...
            Ok(result) => {
                ...
                Ok(Some((
                    result.content_type.unwrap_or("image/jpeg".to_string()),
                    bytes,
                )))
            }
        }
...
async fn create_s3_client(
    ...
    let head_bucket = client.head_bucket().bucket(bucket).send().await;

    if head_bucket.is_err_and(|err| err.into_service_error().is_not_found()) {
...
mod tests {
    ...
    async fn setup_test_backend() -> Backend {
        let db_con =
            PgConnection::establish("postgres://postgres:postgres@127.0.0.1:5432/postgres")
                .unwrap();
        ...
        let s3_client =
            create_s3_client("root", "rootpassword", "http://localhost:9000/", "images").await;
        let backend = Backend {
